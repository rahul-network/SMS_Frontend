...
@Component({
  selector: 'app-billing-submitted',
  templateUrl: './billing-submitted.component.html',
  styleUrls: ['./billing-submitted.component.scss']
})
export class BillingSubmittedComponent implements AfterViewInit {
  moment: any = moment;
  ...
  getBillingCPTCompleted() {
    ...
    this.billingService.getBillingCPTCompleted(this.fromDate.value, this.toDate.value, index, pageSize, this.sort).subscribe(results => {
  ...
  exportBillingCPTSubmitted() {
    ...
    var index = this.paginator ? this.paginator.pageIndex : 0;
    var pageSize = this.paginator ? this.paginator.pageSize : 10;
    var filename = "CPTCodes_Submitted" + moment().format('MMM DD YYYY HH:mm') + ".xlsx";
    this.billingService.exportBillingCptCodes(this.fromDate.value, this.toDate.value, index, pageSize, this.sort, true, false).subscribe(jsonarray => {
      var columns = [
